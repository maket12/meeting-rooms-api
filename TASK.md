...
* Используйте API из файла [api.yaml](backend/api/openapi.yaml). Реализация должна соответствовать описанной в нём спецификации.
...
* Реализуйте GET API метод /_info, который должен возвращать на любой запрос 200.
...
Нужно генерить слоты так как при создании расписания они только на следующие 7 дней. то есть при запросе если нет слотов - генерим